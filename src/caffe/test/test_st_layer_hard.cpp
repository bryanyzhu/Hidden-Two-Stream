...
class HardSpatialTransformerLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  HardSpatialTransformerLayerTest()
 	 : blob_U_(new Blob<Dtype>(2, 3, 5, 5)),
 	   blob_flow_(new Blob<Dtype>(2, 2, 5, 5)),
 	   blob_V_(new Blob<Dtype>(2, 3, 5, 5)) {

	  /*
	  FillerParameter filler_param;
	  GaussianFiller<Dtype> filler(filler_param);
	  filler.Fill(this->blob_U_);
	  filler.Fill(this->blob_flow_);
	  */
  ...
  virtual ~HardSpatialTransformerLayerTest() { delete blob_V_; delete blob_flow_; delete blob_U_; }
  Blob<Dtype>* blob_U_;
  Blob<Dtype>* blob_flow_;
  Blob<Dtype>* blob_V_;
      ...
      sizeof(Dtype) == 4 || IS_VALID_CUDA) {
		...
		*/
		...
		checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
			this->blob_top_vec_,
			1); // check only the gradient for flow
  } else {
