...
// Do RGB and flow images transformation
template<typename Dtype>
void DataTransformer<Dtype>::TransformRgbFlow(const Datum& datum,
  ...
  const int blob_num = transformed_blob->num();

  CHECK_EQ(blob_channels, datum_channels);
  ...
  // The datum contains both rgb and flow images
  const int length = (datum_channels+2)/5;
  const int rgb_channels = length * 3;
  const int flow_channels = (length - 1) * 2;
  ...
  CHECK_GT(datum_channels, 0);
  CHECK_GT(rgb_channels, 0);
  CHECK_GT(flow_channels, 0);
  ...
  if (has_mean_values) {
    CHECK(mean_values_.size() == 4 || mean_values_.size() == datum_channels) <<
     "Specify either 4 mean_value or as many as channels: " << datum_channels;
    if (datum_channels > 1 && mean_values_.size() == 1) {
      // Replicate the mean_value for simplicity
      for (int c = 1; c < datum_channels; ++c) {
        mean_values_.push_back(mean_values_[0]);
    ...
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
            transformed_data[top_index] =
                (datum_element - mean_values_[c%3]) * scale;
          } else {
            transformed_data[top_index] = datum_element * scale;
          }
        }
  ...
  // For the rest flow images
  for (int c = rgb_channels; c < datum_channels; ++c) {
...
// Do multi-frame image pair data transform
template<typename Dtype>
void DataTransformer<Dtype>::TransformMultiFrame(const Datum& datum,
                                       Blob<Dtype>* transformed_blob) {
  ...
  if (has_mean_values) {
    CHECK(mean_values_.size() == 3 || mean_values_.size() == datum_channels) <<
     "Specify either 3 mean_value or as many as channels: " << datum_channels;
    if (datum_channels > 1 && mean_values_.size() == 1) {
      // Replicate the mean_value for simplicity
      for (int c = 1; c < datum_channels; ++c) {
        mean_values_.push_back(mean_values_[0]);
